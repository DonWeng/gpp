...
{
	//When created, set state to active
	setActive(true);
	...
	// title texture
	if (!gameTitleTexture.initialize(graphics, TITLE_IMAGE))
		throw(GameError(gameErrorNS::FATAL_ERROR, "Error initializing title texture"));

	// sky image
	if (!gameTitle.initialize(graphics, 640, 274, 0, &gameTitleTexture))
		throw(GameError(gameErrorNS::FATAL_ERROR, "Error initializing game title"));
